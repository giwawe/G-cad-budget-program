...
本清单用于把当前 CAD 报价工具收口为第一个可试用版本。V1 目标是让设计师基于规范方案文件完成上传、方案完整性复核、工程量校对、报价抬头填写和 Excel 草稿导出；登录、后台管理、合同等平台化能力放到后续版本。V1.1 已补入 DWG 上传入口，服务器需配置转换器后可用。

## 已完成能力

- DXF/DWG 上传解析：DXF 直接解析；DWG 通过服务端 ODA File Converter 转为临时 DXF 后解析，支持规范图层下的空间、墙线、门窗、洞口、吊顶、柜体、洁具、外墙轮廓等算量。
...
小范围试用期间，问题统一记录到 `docs/v1-trial-feedback.md`。先按“必须修 / 近期优化 / 后续版本”分级，再决定后续小版本范围，避免把登录、后台管理等大功能混入当前可用版本收口。

## 后续版本再做

- DWG 转换器安装、部署和失败重试流程深化。
