...
系统会自动判断门类型，尽量减少额外出图工作量。块名或图层名含 `入户`、`进户`、`防盗`、`entry` 判为入户门；含 `推拉`、`移门`、`sliding` 判为推拉门；无关键词且门宽 `>=1.4m` 判为推拉门；剩余普通门判为室内门。只有判为室内门且 `opening_type=normal_door` 的门洞会计入 `interior_door_count`，用于“室内门”按樘计价。入户门、推拉门、疑似大洞口和大洞口不自动计入室内门报价；跨空间门洞如果被两个空间重复归属，需要在校准 JSON 中人工修正。
