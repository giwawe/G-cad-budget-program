...
地面瓷砖主材当前按 750X1500 规格、5% 损耗率从地面面积换算片数，并向上取整；报价映射中按全屋片数汇总生成，不按每个空间拆行。选品规格、拼花、斜铺或特殊损耗仍需要报价规则或校准 JSON 进一步确认。
...
厨房推拉门面积和门套长度在工程量表中展示，并导出到校准模板：`sliding_door_area_m2` 按门宽乘门高，`sliding_door_casing_length_m` 按门宽加两侧门高；当前默认门高为 `2.1m`。卫生间门导出为 `bathroom_door_count`，不混入室内门数量。默认报价规则会分别生成“卫生间门”“厨房推拉门”“厨房推拉门双包套”，但单价暂为 0，报价员可在规则单价表或报价规则 JSON 中补价。
...
报价映射区域会单独展示“全屋汇总项”，集中列出 `space_name="全屋"` 的清单项，例如地砖主材、强电布线、水路布管、砌120厚砖墙、拆改及拆墙、全屋灯饰等。这些项目不在工程量表中按空间展示，但仍会进入导出的报价映射 JSON 和金额汇总。
