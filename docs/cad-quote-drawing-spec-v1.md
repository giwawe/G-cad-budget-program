...
全屋灯饰和全屋插座开关当前作为项目级套餐处理：只要报价映射中存在至少一个可计价空间，就分别生成 1 套“全屋灯饰”和 1 套“全屋插座开关”，不随空间数量重复计费；灯位数量、开关插座数量、品牌配置和套餐差异后续可通过报价规则或点位图层细化。
...
报价映射区域会提示 Excel 草稿包含 21 项“人工补项”；这些项目只用于提醒报价员补填，不写入报价映射 `items`，也不影响 `summary.total_amount`。已自动接入的拆改及拆墙、卫生间门、厨房推拉门、厨房推拉门双包套、材料搬运费、垃圾清运费、地面砖现场维护费、全屋插座开关、美缝和瓷砖加工费不再重复放入人工补项。
