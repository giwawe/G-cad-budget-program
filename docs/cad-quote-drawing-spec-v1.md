...
报价映射区域会提示 Excel 草稿的“Excel 可选补项”数量填写情况；这些项目用于报价员补填或二选一，不写入报价映射 `items`，也不影响 `summary.total_amount`。Excel 草稿会按固定公共大项输出这些人工项或同名模板项；已自动接入的项目会用自动数量和金额替代占位行，设计师在面板中填写数量时会覆盖 Excel 草稿中的同名行数量并计入 Excel 小计和总计。
