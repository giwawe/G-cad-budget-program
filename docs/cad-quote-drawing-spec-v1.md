...
厨房、卫生间顶面是可校对口径：系统默认按集成吊顶处理，使用 `ceiling_area_m2 = 地面面积` 输出“厨房卫生间集成吊顶”候选项，当前默认单价为 260，报价员可在报价规则单价表中修改；如果现场方案选择石膏板吊顶，前端工程量表可切换为石膏板吊顶，此时同一 `ceiling_area_m2` 会进入轻钢龙骨平顶、顶面批嵌和顶面乳胶漆。露台默认视为露天空间，不生成顶面吊顶、顶面批嵌和顶面乳胶漆；即使旧本机报价规则仍把露台写入顶面规则，映射时也会硬性排除露台顶面项。少量有顶面的露台后续由设计师补项或校准。
