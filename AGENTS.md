...
这是一个面向装修报价员的 CAD/DXF 空间算量验证工具。目标是读取按规范绘制的商品房方案文件，自动生成可校对、可追溯的空间工程量表，并逐步承接校准、差异检查和报价映射。当前稳定解析路径仍是 DXF；DWG 上传会先在服务端转换为临时 DXF 后再进入同一解析流程。
...
- `server/app/cad_uploads.py`：CAD 上传归一化边界，负责校验 DXF/DWG 扩展名，并在 DWG 上传时调用 ODA File Converter 转为 DXF。
...
- `POST /api/parse-dxf`：上传 DXF/DWG，返回算量行数组；DWG 会先转换为临时 DXF。
...
- 上传 DXF/DWG 并调用 `8010` 后端解析；DXF 直接解析，DWG 先转换为临时 DXF。
...
- V1.1 前端试用版已把设计师主流程收敛为“方案上传 / 方案完整性复核 / 方案报价可选项 / 水电点位复核 / 空间工程量摘要 / 查看预算”；校准 JSON、快照、报价映射 JSON 和报价规则维护默认收进“后台/校准工具”。
