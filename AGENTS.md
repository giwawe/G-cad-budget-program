...
- 页面会显示“算量健康检查”面板，集中提示可计价空间被识别为“其他”、建筑面积为 0、窗帘/窗帘箱仍需人工确认、卫生间门/室内门/厨房推拉门分类异常、厨房橱柜/全屋定制/卫生间洁具异常，以及依赖建筑面积的报价项未进入金额汇总等问题。检查项分 `warning` 和 `info`：高概率影响报价的空间/建筑面积/门类问题为 warning；厨房推拉门未生成、橱柜缺失、洁具缺失这类可能是正常设计选择的问题为 info；面板标题会汇总显示需优先处理项和提醒项数量。
