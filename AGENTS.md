...
- 导出报价映射 JSON；报价规则单价统一维护，导出时可选择“硬装（半包）”“整装（全包）”或“硬装 + 自选增项包”。硬装模式只输出施工和基础硬装项；整装模式输出全部已接入项目；自选增项包可叠加瓷砖主材/美缝、门窗定制、定制/橱柜、卫浴洁具、开关灯饰、窗帘窗台石、保洁等包。当前选择会同时影响报价映射 JSON 和 Excel 草稿，并写入校对快照；导入快照后恢复。导出时仍有 `warning` 健康检查项、自定义报价规则导致的零单价或建筑面积缺失时，页面会弹出草稿报价确认，确认后仍可继续导出。
