...
- 当前 `curtain_wall_width_m` 只是候选取数：客厅、卧室、书房有窗时优先按窗洞中心线匹配邻近且平行的 `QUOTE_WALL`，取窗户所在墙面的整面墙宽；匹配不到时回退到该空间最长一段 `QUOTE_WALL`；其它空间为 `0`。前端工程量表可人工校准并随校对快照保存/恢复，但它不进入报价规则和金额汇总。
