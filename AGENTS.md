...
- 导出报价映射 JSON；默认使用商品房报价表 `整装` 工作表中当前可自动取数的 8 条规则，跳过不计价空间。
...
商品房整装待补取数口径记录在 `apartmentPendingQuoteMetrics()`，只用于页面展示和后续扩展，不混入可导入规则 JSON，也不参与金额汇总。当前重点包括墙砖、防水、窗台石、窗帘箱、水电、拆改、门、洁具、定制、主材、套装项等。
