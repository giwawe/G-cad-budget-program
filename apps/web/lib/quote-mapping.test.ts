...
assert.equal(mapping.items[2].quantity, 5);
assert.equal(mapping.items[2].amount, 250);
assert.equal(mapping.items[3].quantity, 4.48);
assert.equal(mapping.items[3].amount, 349.44);
assert.equal(mapping.items[4].quantity, 4.48);
assert.equal(mapping.items[4].amount, 132.16);
assert.equal(mapping.items[5].quantity, 20.7);
assert.equal(mapping.items[5].amount, 2070);
assert.equal(mapping.items[6].quantity, 7.22);
assert.equal(mapping.items[6].amount, 371.83);
assert.equal(mapping.items[7].quantity, 4.3);
assert.equal(mapping.items[7].amount, 2580);
assert.equal(mapping.items[8].quantity, 3);
assert.equal(mapping.items[8].amount, 1800);
...
assert.equal(mapping.summary.total_amount, 14234.39);
...
const bedroomDefaultMapping = buildQuoteMapping([{ ...rows[0], spaceName: "主卧", spaceType: "卧室", wallTileAreaM2: 0, waterproofAreaM2: 0, customCabinetAreaM2: 7.2, kitchenBaseCabinetLengthM: 0, kitchenWallCabinetLengthM: 0 }]);
...
assert.equal(bedroomDefaultMapping.summary.total_amount, 13817.76);
...
assert.deepEqual(balconyMapping.items.map((item) => item.item_name), ["地面找平", "地面砖铺贴(750X1500)", "地面瓷砖主材", "强电布线", "水路布管", "墙地面防漏处理", "全屋灯饰"]);

const tiledBalconyMapping = buildQuoteMapping([{ ...rows[0], spaceName: "阳台", spaceType: "阳台", wallTileAreaM2: 14, waterproofAreaM2: 9 }]);
assert.deepEqual(tiledBalconyMapping.items.map((item) => item.item_name), ["地面找平", "地面砖铺贴(750X1500)", "地面瓷砖主材", "强电布线", "水路布管", "墙面贴瓷砖(600X1200)", "墙地面防漏处理", "全屋灯饰"]);
...
const customCabinetMapping = buildQuoteMapping([{ ...rows[0], spaceName: "主卧", spaceType: "卧室", wallTileAreaM2: 0, waterproofAreaM2: 0, customCabinetAreaM2: 7.2 }]);
...
assert.equal(curtainCandidateMapping.summary.total_amount, 16955.28);
